...
The analysis computes high-frequency RMS, low-frequency RMS, FlightGround, hysteresis state transitions, and roll-phase timing in browser JavaScript using constants generated from `config.h`. Landing transitions can be gated by recent high-frequency RMS: when enabled, a FlightGround flight-to-ground transition is accepted only if normalized HIRMS exceeded the configured peak threshold within the configured look-back period before the transition. This prevents calm in-flight FlightGround crossings from being treated as landings without requiring an additional recorder-side data format.
