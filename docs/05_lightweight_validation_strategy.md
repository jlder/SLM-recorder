...
- Displayed current/min stddev values are active-axis values for the current face, and the active-axis value is the value used for recorder face acceptance.
...
Verify that recorder calibration keeps sampling a stable face after the first capture and stores an improved candidate only when the dominant face-axis noise improves. Verify that recorder face acceptance uses only the active calibrated axis (+X/-X -> X, +Y/-Y -> Y, +Z/-Z -> Z), while installation calibration still requires all three axes to be stable. Verify that zero or near-zero stddev values are rejected and cannot be displayed or retained as the best value. Verify that the Web page reports a simplified progress summary: validity status with NVS date when valid, session state, current face, samples processed on the current face, sensor temperature status or the red bold `Temperature too high, Start again` restart instruction, current/min active-axis stddev for the current face, and time since the retained minimum stddev was last improved. Verify that a high sensor temperature or excessive calibration-session temperature span automatically cancels the recorder-calibration session, disables Save, returns the Start/Cancel control to Start, and leaves the restart instruction visible until a new calibration is started or the page is refreshed. Verify that a newly improved minimum is displayed coherently by showing the same accepted-window value as current for that refresh. Verify that the six-face summary shows captured faces as OK and missing faces as —, with unprocessed faces in plain text, the active face in amber only until processed, and processed faces in green. Verify that the workflow text tells the operator to save when all six face values are satisfactory.
