...
- no data after close;
- future calibration block `0x72` once implemented.
...
- Previous valid calibration is not overwritten.
...
### VAL-SD-004 — Web delete archives to processed folder
...
- The file is no longer shown in the active root-file list.
- If the destination name already exists, a `_N` suffix is added before the extension.

### VAL-SD-005 — SD maintenance access when recording is blocked
...
### VAL-SD-006 — SD removal recovery into file-count maintenance
