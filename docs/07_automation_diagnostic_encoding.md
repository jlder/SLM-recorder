...
The encoding described here applies unchanged to the v1.54 and v1.55 field-validation builds with:
...
### 11.1 v1.54 integration finding and v1.55 correction
...
v1.55 implements that requirement in `src/tasks/web_ui/12_script_flight_decode.inc`. The decoder validates each stored 0x70 checksum, restores each axis deterministically using the clean-band threshold and +/-20.001 g offset rule, and then converts milli-g to g for normal browser analysis. The restoration step does not reject or remove checksum-valid 0x70 samples. No diagnostic event is removed from the file itself.

## 12. Separation from live automation behavior

The encoding does not feed back into the automation detectors.

The intended v1.55 analysis flow is:
...
For the v1.54/v1.55 field-validation builds the physical accelerometer range makes the diagnostic bands self-identifying at the individual-axis level: valid physical samples cannot normally exceed the reserved +/-10 g clean band.
