...
static bool calibration_to_driver_(const calibration_record_t *rec, accel_calibration_t *out){
  ...
  out->gain_x = rec->gain_x;
  out->gain_y = rec->gain_y;
  out->gain_z = rec->gain_z;
  out->offset_x_mg = rec->offset_x_mg;
  out->offset_y_mg = rec->offset_y_mg;
  out->offset_z_mg = rec->offset_z_mg;
  return true;
}
...
void calibration_service_refresh_status(void){
  ...
  if(!s_active_loaded || !s_active_cal.valid){
    s_status = CAL_STATUS_MISSING;
    calibration_apply_driver_state_();
    return;
  }
  ...
  if(calibration_is_expired_(&s_active_cal.timestamp, &now)){
...
bool calibration_service_is_recording_allowed(void){
  calibration_service_refresh_status();
  return (s_status == CAL_STATUS_VALID);
}
...
static void calibration_copy_face_status_(calibration_sample_status_t *out){
  ...
  for(uint32_t i = 0u; i < (uint32_t)CAL_FACE_COUNT; ++i){
    ...
    if(s_active_loaded){
      out->stored_face[i] = s_active_cal.face[i];
    }
  }
...
bool calibration_session_compute(calibration_record_t *out){
  ...
  rec.valid = true;
  rec.timestamp = now;
  rec.gravity_mg = CALIBRATION_GRAVITY_MG;

  rec.gain_x = (2.0f * CALIBRATION_GRAVITY_MG) / (x_pos - x_neg);
  rec.gain_y = (2.0f * CALIBRATION_GRAVITY_MG) / (y_pos - y_neg);
  rec.gain_z = (2.0f * CALIBRATION_GRAVITY_MG) / (z_pos - z_neg);

  rec.offset_x_mg = -rec.gain_x * ((x_pos + x_neg) * 0.5f);
  rec.offset_y_mg = -rec.gain_y * ((y_pos + y_neg) * 0.5f);
  rec.offset_z_mg = -rec.gain_z * ((z_pos + z_neg) * 0.5f);

  for(uint32_t i = 0u; i < (uint32_t)CAL_FACE_COUNT; ++i){
    rec.face[i] = s_session_face[i];
  }

  if(!calibration_gain_offset_plausible_(rec.gain_x, rec.offset_x_mg) ||
     !calibration_gain_offset_plausible_(rec.gain_y, rec.offset_y_mg) ||
     !calibration_gain_offset_plausible_(rec.gain_z, rec.offset_z_mg)){
    calibration_service_latch_fault();
    return false;
  }
